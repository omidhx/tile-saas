...
| قیمت‌گذاری + تخفیف حجمی | ✅ v2 — `pricing.ts`, `/staff/prices`. قیمت در لحظه‌ی تأیید **snapshot** می‌شود. v9: ساختِ سبدِ قیمت‌گذاری (`/api/price-lists`) + ورودِ اکسلِ دسته‌جمعی (`/staff/catalog?tab=priceImport`) هم از UI. **باز:** افزودن پله‌ی تخفیف از UI (فعلاً SQL) | spec ۵.۷ |
