...
| `/api/metrics` | مدیرِ پلتفرم (`app_user.is_platform_admin`) در production | **Metrics عملیاتی** — request count per route، latency (avg/min/max)، 4xx/5xx count، error rate، uptime. **(Phase 8)** بخش `backup` اضافه شد: `last_success_at`، `last_failure_at`، `last_failure_reason`، `last_success_size_bytes`، `last_success_sha256` (۱۶ کاراکتر اول)، `backup_age_seconds`، `restore_test_last_success_at`، `retention_days`. اگر status file موجود نباشد: `{ configured: false }`. هیچ secret یا path حساسی expose نمی‌شود. |
