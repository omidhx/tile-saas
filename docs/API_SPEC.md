...
| `/api/reports` | staff | گزارش‌های مدیریتی (عملکردِ نماینده، پرفروش/راکد، تخفیف‌های اعمال‌شده به‌تفکیکِ نماینده/کالا)؛ `&agentAccountId`/`&variantId` اختیاری برای محدودکردنِ همان گزارش به یک نماینده/کالا (راکدها فقط فیلترِ کالا را می‌پذیرد). `&monthly=1[&months]` همان مسیر را عوض می‌کند به عملکردِ نماینده ماه‌به‌ماهِ شمسی (پیش‌فرض ۶ ماه، سقفِ ۱۲) |
