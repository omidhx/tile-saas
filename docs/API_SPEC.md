...
| `/api/metrics` | مدیرِ پلتفرم (`app_user.is_platform_admin`) در production | **Metrics عملیاتی** — request count per route، latency (avg/min/max)، 4xx/5xx count، error rate، uptime. در dev بدون auth. |
