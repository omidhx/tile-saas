...
| افشای وجود کاربر | پیامِ یکسان برای هر شکست + verify روی هشِ الکی حتی وقتی کاربر نیست (ضدِ تحلیلِ زمان‌بندی) | `api/auth/login` |
| خروج | `POST` نه `GET` — با `<img>` سایتِ دیگر نمی‌شود کاربر را خارج کرد | `api/auth/logout` |
...
**نقاط ضعفِ پذیرفته‌شده:**
- **«خروج از همه‌ی دستگاه‌ها» وجود ندارد.** JWT بی‌حالت است؛ توکنِ دزدیده‌شده تا انقضا (۷ روز) معتبر است. برای رفعش `session_version` روی `app_user` لازم است. → وقتی تغییر رمز اضافه شد.
- **تغییر/بازیابی رمز وجود ندارد.** فعلاً staff دستی ریست می‌کند.
- **rate limiter در حافظه‌ی پروسه است.** با چند instance سقف ضرب می‌شود و با ری‌استارت صفر. spec عمداً Redis را در این مقیاس رد کرده. → اگر multi-instance شدی، به Postgres منتقلش کن.
...
- **آپلود فایل:** **هیچ فایلی به سرور نمی‌رسد.** اکسل در **مرورگر** با SheetJS پارس می‌شود و فقط JSON ارسال می‌گردد — یعنی کلِ سطحِ حمله‌ی upload (حجم، MIME، ماکرو، مسیر) حذف شده، نه مدیریت. سقفِ ۵۰۰۰ ردیف روی JSON اعمال می‌شود.
...
| ادعای نشتِ بین‌تننتی | تأیید کن اپ با نقشِ non-superuser وصل است | `scripts/prodlike-smoke.sh` را اجرا کن؛ تست‌های IDOR را ببین |
...
| نشست لو رفته | `AUTH_SECRET` را عوض کن | همه خارج می‌شوند (تا وقتی «خروج از همه‌جا» ساخته شود) |

---

## ۹. آنچه هنوز نداریم (صادقانه)

- تغییر/بازیابی رمز، و «خروج از همه‌ی دستگاه‌ها».
- 2FA / OTP پیامکی برای ورود.
- `audit_log` نوشته می‌شود ولی هیچ UI برای مرورش نیست (لجرِ موجودی UI دارد).
- تفکیکِ ریزترِ نقش‌ها (فعلاً `admin`/`staff`/`agent`؛ `agent_admin` در برابر `agent_operator` نداریم).
- پایشِ خودکار و هشدار (spec عمداً Prometheus را در این مرحله رد کرده؛ فعلاً لاگ + بررسیِ هفتگی).
- اسکنِ خودکارِ وابستگی‌ها (`npm audit` دستی).

## ۱۰. قبل از هر دیپلوی

```bash
bash scripts/prodlike-smoke.sh   # اپ با نقشِ محدود و RLS واقعاً فعال
cd web && npm test               # ۳۸ تست، شامل IDOR و یکپارچگی موجودی
```
