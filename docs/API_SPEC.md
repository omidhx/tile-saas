...
| `/api/reports` | staff | گزارش‌های مدیریتی (عملکردِ نماینده، پرفروش/راکد، تخفیف‌های اعمال‌شده به‌تفکیکِ نماینده/کالا)؛ `&agentAccountId`/`&variantId` اختیاری برای محدودکردنِ همان گزارش به یک نماینده/کالا (راکدها فقط فیلترِ کالا را می‌پذیرد) |
