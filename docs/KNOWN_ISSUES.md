# KNOWN_ISSUES

محدودیت‌ها و کارهای باز که **آگاهانه** فعلاً نیستند — تا شفاف بمانند و کسی فکر نکند فراموش شده‌اند. (باگِ فعال نداریم؛ این‌ها gapهای دامنه/پیاده‌سازی‌اند.)

## پیاده‌سازی (ساخته نشده)
| مورد | وضعیت | مرجع |
|---|---|---|
| تأیید SalesRequest (`held→allocated`) | ✅ ساخته و تست‌شده؛ **staff-only** (`authorizeStaff`) | spec ۱۴.۲ |
| SalesDispatch + state machine + `loaded` | ✅ ساخته و تست‌شده؛ create-از-request + loaded + cancel-release، **staff-only** | spec ۱۴.۳ |
| مسیر backorder (محصول ناموجود) | ✅ ساخته و تست‌شده (`createBackorderDispatch`/`setBackorderItemStatus`, `/api/backorders`, پنل staff)؛ کاملاً بیرون از موجودی (تست: on_hand دست‌نخورده، لجر صفر). **باز:** تبدیلِ backorder به in_stock وقتی تولید شد | spec ۵.۶ |
| نقش staff/agent | ✅ `tenant_membership.role` (admin/staff/agent) با CHECK؛ تأیید+حواله staff-only، نماینده ۴۰۳ (unit + e2e). **باز:** تفکیک agent_admin/operator | spec ۱۴.۶ |
| Import اکسل (snapshot) | ✅ ساخته و تست‌شده (`imports.ts`, `/api/imports`, `/staff/import` با پارسِ مرورگریِ SheetJS — فایل به سرور نمی‌رسه). scope-aware zeroing، guardِ below_committed/absent_but_committed، اتمیک، idempotent. **باز:** import_mode=delta (فقط snapshot)، natural-key نهایی وابسته به مصاحبه | spec ۱۴.۵ |
| «رزروهای من» + دکمه‌ی تأیید | ✅ صفحه `/reservations` | — |
| پنل staff (حواله + بارگیری) | ✅ صفحه `/staff` (درخواست‌های تأییدشده → حواله → گذارِ وضعیت). v9: صفِ رزرو/درخواست هر ۶۰ ثانیه خودکار تازه می‌شود (همان الگویِ pollingِ «رزروهای من»ِ نماینده) + اعلانِ صوتی/بصریِ رزرو/درخواستِ تازه (بوقِ Web Audio، کارتِ هایلایت‌شده با نشانِ «تازه»، بنرِ بالای صفحه، دکمه‌ی خاموش/روشن‌کردنِ صدا با localStorage) | — |
| سوییچرِ چند-نمایندگی | ✅ `useContexts` + `ContextSwitcher`؛ انتخاب در localStorage می‌ماند (سرور هر درخواست را دوباره authorize می‌کند، پس دست‌کاری‌اش فقط ۴۰۳ می‌دهد). وقتی فقط یک context باشد چیزی نشان داده نمی‌شود | — |
| worker انقضا | ✅ `expire_due_reservations()` + `npm run worker:expire` (cron هر ۱۰-۱۵ دقیقه) | spec ۵.۳ |
| StockAlert + پیامک (Outbox) | ✅ `alerts.ts`/`outbox.ts`، `/api/alerts`، `npm run worker:outbox`. صف‌کردن اتمیک با import، claim-then-send، retry×۵ و dead-letter. **باز:** ارائه‌دهنده‌ی واقعی پیامک (فعلاً `SMS_PROVIDER=log`) | spec ۵.۹ |
| قیمت‌گذاری + تخفیف حجمی | ✅ v2 — `pricing.ts`, `/staff/prices`. قیمت در لحظه‌ی تأیید **snapshot** می‌شود. v9: ساختِ سبدِ قیمت‌گذاری (`/api/price-lists`) + ورودِ اکسلِ دسته‌جمعی (`/staff/catalog?tab=priceImport`) + مدیریتِ کاملِ پله‌های تخفیفِ حجمی (`/staff/catalog?tab=volumeDiscount`) + استثنای قیمتِ نمایندگی‌محور (`agent_price_override`، از کارتِ نماینده در `/staff/agents`) هم از UI؛ همه‌جا CRUD واقعی، نه فقط SQL | spec ۵.۷ |
| تأیید هیبریدی (سقف خودکار) | ✅ v2 — سقفِ tenant/agent، پیش‌فرض **خاموش**، هر ابهامی → تأییدِ دستی. `/staff/auto-approve` | spec ۹ |
| صف انتظار | ✅ v2 — به‌ترتیبِ نوبت، در همان تراکنشِ آزادسازی | spec ۹ |
| چندانباره | ✅ v2 — سفارشِ دوانباره خودکار به دو حواله تقسیم می‌شود | spec ۹ |
| کالای جایگزین | ✅ v2 — تعریفِ صریح + «همان محصول، درجه‌ی دیگر». هرگز جایگزینِ ناموجود پیشنهاد نمی‌شود | spec ۹ |
| موجودی در راه | ✅ v2 — **هرگز وارد `available` نمی‌شود**؛ رسیدن از مسیرِ لجر. `/staff/incoming` | spec ۹ |
| گزارش‌های مدیریتی | ✅ v2 — عملکرد نماینده، پرفروش، راکد. ارزش از snapshot، کارتن از لجر. v9: تخفیف‌های اعمال‌شده (از `discount_amount`ِ همان snapshot) به‌تفکیکِ نماینده و کالا + فیلترِ اختیاریِ نماینده/کالا رویِ کلِ گزارش (راکدها فقط فیلترِ کالا را می‌پذیرد، چون موجودی مالِ نماینده‌ی خاصی نیست) + عملکردِ نماینده ماه‌به‌ماهِ شمسی (۳/۶/۱۲ ماهِ اخیر) + عملکردِ نماینده به‌تفکیکِ انبار (از لجرِ dispatch_load، فقط کارتن — سفارش می‌تواند از چند انبار پر شود پس ارزش به یک انبار snapshot نمی‌شود) | spec ۹ |
| مشتری به‌عنوان Entity | ✅ v2 — با تاریخچه و گزارشِ پرخریدترین. نام روی حواله snapshot می‌ماند | spec ۹/۷.۷ |
| تغییر/بازیابی رمز + خروج از همه | ✅ `session_epoch`؛ بازیابی با کدِ پیامکیِ یک‌بارمصرف | — |
| دفتر تغییراتِ قواعدِ پولی | ✅ `audit_log` — قیمت، سقفِ تأیید (کارخانه و نمایندگی)، و v10: ویرایشِ محصول/مشتری هم اضافه شد (فقط فیلدهای واقعاً تغییرکرده) | — |
| مانیتورینگِ خطا | ✅ v10 — `@sentry/nextjs`؛ هر authorize موفق برچسبِ tenantId/userId می‌زند (فقط شناسه) تا ردیابیِ نشتِ بین‌تننتی سریع‌تر شود | — |
| ~~نقشِ `admin` معنا ندارد~~ | ✅ حل شد (v4): `admin` حالا تنها نقشی است که می‌تواند تیم/نمایندگی‌ها/انبارها را مدیریت کند، و `can_manage_access` (فقط رویِ admin معنا دارد) مشخص می‌کند چه کسی حق دارد دسترسیِ ریزدانه‌ی بقیه را تنظیم کند | — |
| دسترسیِ ریزدانه‌ی سطحِ صفحه (staff) | ✅ v4 — `tenant_membership.allowed_pages` (خالی=کامل) + `authorizeStaffPage`؛ صفحه‌ی بی‌دسترسی در NavMenu اصلاً نشان داده نمی‌شود (نه نمایش-بعد-منع). ده صفحه‌ی عملیاتی گیت‌شده‌اند؛ `/staff` و صفحاتِ ادمین‌محور (تیم/نمایندگی/انبار) مستثنا | — |
| حذفِ واقعیِ تیم/نمایندگی/انبار | ✅ v4 — گاردِ «سابقه‌دار بودن» (رد می‌کند نه silent no-op) + گاردهای اختصاصی (آخرین admin/مدیرِ دسترسی، عضوِ متصل به نمایندگی) | — |
| پشتیبانِ ثابتِ نمایندگی (Account Manager) | ✅ v5 — `agent_account.assigned_staff_user_id`؛ نام/شماره هم در `/reservations` (نماینده) هم در صفِ تأیید/درخواستِ پشتیبان، حتی برای تأییدِ خودکار. **باز:** چت آنلاین (فازِ بعد) | — |
| موجودیِ اولیه هنگامِ ساختِ محصول | ✅ v7 — ساختِ محصول در `/staff/catalog` حالا انبار+تعداد اختیاری می‌گیرد و از همان مسیرِ لجرِ import/incoming عبور می‌کند (`transaction_type='initial_stock'`) | — |
| ادغامِ صفحاتِ پنلِ پشتیبان | ✅ v6/v7 — کاتالوگ/قیمت‌گذاری/ورودِ اکسل/موجودیِ در راه/جایگزین زیرِ `/staff/catalog` (تب‌دار)؛ گزارش‌ها/دفترِ حرکات/دفترِ تغییرات زیرِ `/staff/reports`. مسیرهای قدیمی ریدایرکت می‌شوند؛ دسترسیِ ریزدانه دست‌نخورده (چک در سطحِ API، نه route) | — |
| اعلان‌های چندکاناله | ✅ v3 — `notification_outbox.channel` (sms/email/bale)، هرکدام فرستنده‌ی pluggableِ خودش (پیش‌فرض `log`). **باز:** ارائه‌دهنده‌ی واقعیِ هر سه | — |
| ~~روزِ صفرِ tenantِ تازه فقط با SQL~~ | ✅ حل شد (v9): `app_user.is_platform_admin` + `/platform/tenants` — مدیرِ پلتفرم کارخانه + اولینِ مدیرش را از UI می‌سازد (مدیرِ اول همان‌جا `can_manage_access=true` هم دارد)؛ دیگر هیچ SQL دستی‌ای برای مشتریِ تازه لازم نیست | — |
| ~~بدونِ نمای چاپیِ حواله~~ | ✅ حل شد (v9): `/staff/dispatch/:id/print` — کد/اقلام/بچ/شید/کالیبر/محلِ انبار + خطِ امضاء، `@media print` بدونِ رنگ/دکمه | — |
| ~~`LIMIT` ثابت روی لیست‌های append-only~~ | ✅ حل شد (v9): حواله‌ها/backorderها/لجر/دفترِ تغییرات صفحه‌بندی‌شده (`limit+1` → `hasMore`، دکمه‌ی «بیشتر») + جستجو (`&q`، ILIKE روی کد/نام/بچ/نمایندگی). صف‌های زنده (رزروِ در انتظار، درخواستِ تأییدشده) عمداً دست‌نخورده ماندند — با فیلترِ status بسته می‌مانند، مشکلِ رشدِ بی‌پایان ندارند | — |
| ~~بدونِ خروجیِ اکسل/PDF~~ | ✅ حل شد (v9): گزارش‌های مدیریتی/دفترِ حرکات/مشتریان هرکدام «خروجیِ اکسل» (کاملاً کلاینتی، همان `xlsx`ِ import) + «خروجیِ PDF» (چاپِ مرورگر با `@media print`، بدونِ کتابخانه‌ی تازه) دارند. اکسلِ لجر همیشه کلِ نتیجه‌ی فیلترشده را می‌گیرد (`&limit`)، نه فقط صفحه‌ی روی صفحه | — |
| بازطراحیِ Shell/Sidebar/تمِ تیره‌روشن | ✅ v11 — `PageShell`+`ThemeToggle` روی همه‌ی صفحاتِ داخلیِ پشتیبان و نماینده (داشبورد، محصولات، گزارش‌ها، تیم، مشتریان، رزرو/سبد/رزروهای‌من/کاتالوگ‌ها، احرازِهویت)؛ چاپ و کاتالوگِ عمومی عمداً بدونِ Sidebar. تمِ ذخیره‌شده در `localStorage`، بدونِ flash | — |
| تکمیلِ فیلدهای ساختِ حواله (مقصد/مشتری/مرجع) | ✅ v11 — `destination`/`customerId`/`referenceNumber` در `CustomerPicker` + فرمِ ساختِ حواله (`QueueSection`/`BackorderSection`)؛ در `listDispatches`/جزئیات/برگه‌ی چاپ نمایش داده می‌شوند | — |
| نمایشِ واحدِ پول (ریال/تومان) + مبلغِ به حروف | ✅ v11 — تنظیمِ tenant برای واحدِ نمایش؛ مبلغِ رزرو/حواله هم به عدد هم به حروف نشان داده می‌شود | — |

## باز — واقعاً کم است، نه تصمیم
- ~~**CI نداریم.** تست‌ها فقط با `npm test` دستی معتبرند؛ هیچ‌چیز جلوی مرجِ کدِ تست‌نشکسته را نمی‌گیرد.~~ ✅ رفع شد (Phase 10-post): `.github/workflows/ci.yml` با PostgreSQL داکری، typecheck، test، build.
- **AUD-009 (Phase 8→9):** فایل‌های آپلودشده در `private/uploads/` (عکسِ محصول، فاکتورها، اسناد) توسطِ سیستمِ backup فاز ۸ پشتیبانی **نمی‌شدند**. این فایل‌ها روی Docker named volume (`uploads`) ذخیره می‌شدند و اگر volume خراب شود، از بین می‌رفتند. **Phase 9 رفع شد:** اسکریپت‌های `scripts/backup-uploads.sh`، `scripts/verify-uploads.sh`، `scripts/restore-uploads.sh`، و `scripts/cleanup-old-uploads-backups.sh` ساخته شدند. این اسکریپت‌ها از الگوی backup دیتابیس پیروی می‌کنند (tar + zstd + GPG + SHA-256 + manifest + flock + CLEANUP_DONE guard). **وضعیت فعلی:** Implementation complete، static verification complete، CI runtime pending (نیازمند Docker برای تست واقعی). مرجع: `docs/BACKUP_POLICY.md` بخش uploads و `docs/UPLOADS_RESTORE_RUNBOOK.md`.

## تصمیم‌های آگاهانه (نه باگ)
- **رزرو lot-based** (نماینده Lot را می‌بیند). auto-pickِ FIFO مقیدِ شید = v2. (spec ۱۴.۱۱)
- ~~**بدون `db/migrations/`** — چون prod نداریم؛ `schema.sql` نصب مرجع است. اولین دیپلوی = ساخت پوشه‌ی migration. (spec ۱۴.۹)~~ ✅ رفع شد (Phase 10-post): `db/migrations/` با `apply.ts` و دو migration (`0002_migrations_table.sql`, `0003_rate_limit_table.sql`) ساخته شد. `schema.sql` همچنان منبع حقیقت برای نصبِ تازه است.
- **بدون آرشیو/پارتیشن رزرو** — مقیاس این پروژه لازمش ندارد؛ index جزئی کافی است. trigger بازبینی: هزاران رزرو فعالِ هم‌زمان. (spec ۱۴.۱۱)
- **بدون Tailwind/React Query/Zustand** — YAGNI تا وقتی چند صفحه/چند-route شد.
- **Rate limiter در حالتِ `memory` همچنان درون‌پروسه‌ای است** — ولی حالا `RATE_LIMIT_BACKEND=postgres` هم برای multi-instance موجود است. ponytail: برای تک-instance همون `memory` کافی است.

## اسنادِ ساخته‌نشده (به‌محض نیاز ساخته می‌شوند)
`DEPLOYMENT_RUNBOOK.md`, `TESTING_STRATEGY.md`, `DESIGN_SYSTEM.md`, `SEO_PERFORMANCE.md`, `IMPLEMENTATION_PLAN.md`, `openapi.yaml`.
> (`SECURITY.md` و `GO_LIVE.md` ساخته شده‌اند. سیستمِ طراحی فعلاً داخلِ `web/src/app/globals.css` مستند است — توکن‌ها و دلیلِ هر تصمیم در کامنت‌ها.)
> فعلاً محتوای این‌ها در spec (۸ امنیت، ۳/۱۴.۱۰ دیپلوی) و CODING_STANDARDS/API_SPEC پوشش داده شده. وقتی موضوعشان واقعاً پیش آمد (اولین دیپلوی، اولین اختلاف امنیتی، سیستم دیزاینِ جدی) سندِ مستقلش ساخته می‌شود — تا سندِ کهنه‌ی زودهنگام نداشته باشیم.

## وابسته به مصاحبه‌ی کارخانه (spec ۷)
شید/کالیبر (آیا دیجیتال ثبت می‌شود؟)، snapshot vs delta، برند (product یا lot)، natural key برای import. پیش‌فرضِ تحقیق‌شده دارند، قطعی نیستند.

## محیطی
- تست‌ها Docker می‌خواهند؛ اگر Docker Desktop پایین باشد، تست‌های DB fail می‌دهند (نه باگِ کد).
- CRLF/LF: گیت روی ویندوز warning می‌دهد؛ بی‌اثر.
